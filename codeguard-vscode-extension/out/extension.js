...
async function pollForCompletion(analysisId, apiUrl, onProgress) {
  ...
  const final = await axios_default.get(
    `${apiUrl}/api/analysis/${analysisId}`,
    { timeout: 3e4 }
  );
  return final.data;
  ...
  try {
    const response = await axios_default.post(
      ...
      request,
      {
        ...
        timeout: 6e4
        // 60 s — backend now returns in <2 s
      }
    );
...
// src/webview/SidePanel.ts
var CodeGuardPanel = class {
  constructor(_extensionUri) {
    this._extensionUri = _extensionUri;
    this._decorationType = vscode2.window.createTextEditorDecorationType({
  ...
  static {
    this.viewType = "codeguard.sidePanel";
  }
  ...
  async handleAnalyzeRequest(data) {
    try {
      ...
      if (!code.trim()) {
        vscode2.window.showWarningMessage("File is empty!");
        return;
      }
      ...
      const config = vscode2.workspace.getConfiguration("codeguard");
      const useLocal = config.get("useLocalBackend", false);
      const apiUrl = config.get("apiUrl", "");
      if (!useLocal && apiUrl.includes("render.com")) {
        vscode2.window.showInformationMessage("Connecting to backend... First request may take 30-50 seconds (cold start)");
      }
      this.showLoading();
      ...
      if (result.has_bugs) {
        ...
        vscode2.window.showInformationMessage("CodeGuard: No obvious bugs detected");
      }
    } catch (error) {
      this._view?.webview.postMessage({ command: "analysisComplete" });
      vscode2.window.showErrorMessage(`Analysis failed: ${error.message}`);
    }
  }
  async handleFeedbackSubmission(feedbackData) {
    try {
      ...
      vscode2.window.showInformationMessage("Thank you for your feedback!");
    } catch (error) {
      this._view?.webview.postMessage({
        command: "feedbackError",
        error: error.message
      });
      vscode2.window.showErrorMessage(`Failed to submit feedback: ${error.message}`);
    }
  }
  showLoading() {
    if (this._view) {
...
function activate(context) {
  ...
  const provider = new CodeGuardPanel(context.extensionUri);
  ...
  let analyzeFileCommand = vscode3.commands.registerCommand(
    "codeguard.analyzeCode",
    async () => {
      ...
      if (editor.document.languageId !== "python") {
        const proceed = await vscode3.window.showWarningMessage(
          "CodeGuard is optimized for Python. Continue anyway?",
          "Yes",
          "No"
        );
      ...
      await vscode3.window.withProgress({
        ...
        try {
          ...
          if (result.has_bugs) {
            vscode3.window.showWarningMessage(
              ...
              if (selection === "View Details") {
                vscode3.commands.executeCommand("codeguard.sidePanel.focus");
              }
            });
          } else {
          ...
          vscode3.window.showErrorMessage(
            `CodeGuard analysis failed: ${error.message}`
          );
        }
      });
    }
  );
  let analyzeSelectionCommand = vscode3.commands.registerCommand(
    "codeguard.analyzeSelection",
    async () => {
      const editor = vscode3.window.activeTextEditor;
      if (!editor) return;
      ...
      if (!code) {
        vscode3.window.showWarningMessage("No code selected!");
        return;
      }
      const prompt = await vscode3.window.showInputBox({
